...
class RichReport:
    ...
    def _build_styles(self):
        ...
        add("section", fontName="Helvetica-Bold", fontSize=8.5, leading=11, textColor=ORANGE)
        ...
        add("num", fontName="Helvetica-Bold", fontSize=12, leading=14, textColor=ORANGE)
        ...
        for label, valor, cor in cards:
            inner = Table(
                [
                    ...
                    [Paragraph(_esc(valor), self.s["card_value"])],
                ]
            )
            inner.setStyle(
                TableStyle(
    ...
    def conversion_cards(self, cards):
        """cards: lista de (pct_texto, label, sublabel)."""
        celulas = []
        for pct, label, sub in cards:
            inner = Table(
                [
                    [Paragraph(_esc(pct), self.s["conv_pct"])],
    ...
    def callout(self, texto, accent=ORANGE):
    ...
    def two_panels(self, left_title, left_rows, right_title, right_rows):
        ...
        def painel(titulo, rows):
            ...
            for row in rows:
                ...
                dados.append([esquerda, Paragraph(_esc(valor), self.s["panel_value"])])

            t = Table(dados, colWidths=[self.width * 0.30, self.width * 0.14])
            estilo = [
                ("BOX", (0, 0), (-1, -1), 0.5, BORDER),
                ("SPAN", (0, 0), (1, 0)),
                ("LEFTPADDING", (0, 0), (-1, -1), 12),
    ...
    def numbered_list(self, itens, accent=ORANGE):
        ...
        for idx, (lead, resto) in enumerate(itens, start=1):
            ...
            t = Table([[num, texto]], colWidths=[1.0 * cm, self.width - 1.0 * cm])
            t.setStyle(
                TableStyle(
                    [
                        ("BOX", (0, 0), (-1, -1), 0.5, BORDER),
